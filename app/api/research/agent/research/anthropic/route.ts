    ...
    const readableStream = new ReadableStream({
      async start(controller) {
        try {
          ...
          // Claude 3.7 Sonnet에서만 thinking 옵션 추가
          if (reasoningType === "Thinking") {
            streamOptions.thinking = { type: "enabled", budget_tokens: 1600 }
          } else if (reasoningType !== "Thinking") {
            streamOptions.top_p = modelConfig.topP
          }
          ...
          console.log("Starting Anthropic Research stream execution with model:", selectedModel, "reasoningType:", reasoningType)
